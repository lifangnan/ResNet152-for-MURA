...
if __name__=='__main__':
    ...
    study_data = get_study_level_data(study_type='XR_WRIST')  #选择某一部位的图像，返回值是一个字典，有train和valid两类，value是dataframe数据

    # #### Create dataloaders pipeline
    data_cat = ['train', 'valid'] # data categories
    dataloaders = get_dataloaders(study_data, batch_size=1)
    dataset_sizes = {x: len(study_data[x]) for x in data_cat} #样本的数量
    ...
    torch.save(model.state_dict(), 'D:/大三下/领域数据学/models_resnet/model.pth')
