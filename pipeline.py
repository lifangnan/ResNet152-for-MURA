...
def get_study_level_data(study_type):
    ...
    for phase in data_cat:
        BASE_DIR = 'D:/大三下/领域数据学/MURA_test/%s/%s/' % (phase, study_type) #study_type如XR_FINGER
...
def get_dataloaders(data, batch_size=1, study_level=False):
    ...
    data_transforms = {
        'train': transforms.Compose([
                ...
                transforms.ToTensor(),
                transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # 第一个参数是RGB的mean，第二个参数是RGB的std
        ]),
