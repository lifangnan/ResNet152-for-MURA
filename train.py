    ...
    for epoch in range(num_epochs):
        ...
        for phase in data_cat:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                preds2 = preds
                # 因为torch.mean()返回的是标量，需转化为一维张量
                preds2 = torch.Tensor([preds2.tolist()]).cuda()
                running_corrects += torch.sum(preds2 == labels.data)
                confusion_matrix[phase].add(preds2, labels.data)
            epoch_loss = running_loss / dataset_sizes[phase]
            epoch_acc = running_corrects.tolist() / dataset_sizes[phase]
...
def get_metrics(model, criterion, dataloaders, dataset_sizes, phase='valid'):
    ...
    for i, data in enumerate(dataloaders[phase]):
        ...
        preds = (outputs.data > 0.5).type(torch.cuda.FloatTensor)
        preds = torch.Tensor([preds.tolist()]).cuda()
        running_corrects += torch.sum(preds == labels.data)
        confusion_matrix.add(preds, labels.data)

    loss = running_loss / dataset_sizes[phase]
    acc = running_corrects.tolist() / dataset_sizes[phase]
